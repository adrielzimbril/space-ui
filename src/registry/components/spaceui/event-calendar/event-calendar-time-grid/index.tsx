...
function useNow(intervalMs = 30_000): Date {
  ...
  useEventListener('visibilitychange', () => setNow(new Date()), typeof document !== 'undefined' ? document : null)
  useEventListener('focus', () => setNow(new Date()))
...
function EventCalendarTimeGrid({
  ...
  const days = useMemo(() => {
    ...
    const filtered = result.filter((day) => !settings.weekendDays.includes(toZoned(day, settings.timeZone).getDay()))
  ...
  useEffect(() => {
    ...
    instance.internals.registerScrollHandler((time) => {
      const minutes =
        typeof time === 'number'
          ? time
          : toZoned(time, settings.timeZone).getHours() * 60 + toZoned(time, settings.timeZone).getMinutes()
      scrollTo(minutes)
    })
    ...
    const syncScrollbarGutter = () => {
      root?.style.setProperty('--ec-scrollbar-w', `${viewport ? viewport.offsetWidth - viewport.clientWidth : 0}px`)
    }
    syncScrollbarGutter()
  ...
  const allDaySegments = useEventCalendarSelector<unknown, EventCalendarSegment[]>(
    ...
    {
      isEqual: (a, b) => a === b || (a.length === b.length && a.every((segment, i) => segment === b[i])),
    },
  )
  ...
  const track = (
    <div className="relative flex">
      ...
      {effective.nowIndicator && <EventCalendarNowIndicator days={days} startHour={startHour} endHour={endHour} />}
    </div>
  )
      ...
      <>
        ...
        {showAllDay && (
          <div
            ...
            className={cn('flex border-b pe-(--ec-scrollbar-w,0px)', viewConfig.classNames?.allDaySection)}
          >
        ...
        {contained ? (
          <div ref={scrollRef} className="min-h-0 flex-1">
            {viewConfig.scrollbars === 'native' ? (
              <div data-slot="scroll-area-viewport" data-ec-native-scroll="" className="h-full overflow-y-auto">
                {track}
              </div>
            ) : (
 ...
 */
function EventCalendarAllDayBars({ days, gridTemplateColumns }: { days: Date[]; gridTemplateColumns: string }) {
  const settings = useEventCalendarSettings()
  const viewConfig = useEventCalendarViewConfig()
  ...
  const dragGhost = useEventCalendarSelector<
    unknown,
    | (Pick<EventCalendarDragState, 'kind' | 'valid' | 'occurrence' | 'proposedStart' | 'proposedEnd'> & {
        colStart: number
        colSpan: number
        isStart: boolean
        isEnd: boolean
  ...
  const ghostLane = dragGhost ? (bars.find((s) => s.occurrence.key === dragGhost.occurrence.key)?.lane ?? laneCount) : 0
      ...
      {(bars.length > 0 || dragGhost) && (
        ...
        >
          {bars.map((segment) => (
            ...
            >
              <EventCalendarEvent segment={segment} className="h-[calc(var(--ec-month-bar-h,1.625rem)-0.125rem)]" />
            </div>
          ))}
          {dragGhost && (
            <div
...
function EventCalendarTimeGutter({
  ...
  const labelFormat = interval % 60 === 0 ? settings.i18n.formats.timeGutter : settings.i18n.formats.timeGutterMinute
  return (
    <div
      data-slot="event-calendar-time-gutter"
      className={cn('relative w-(--ec-gutter-width,4.5rem) shrink-0 border-e', viewConfig.classNames?.timeGutter)}
    >
      {slots.map((minutes) => {
        ...
        const label =
          viewConfig.renderTimeGutterSlot?.({
            ...
            minute: minutes % 60,
          }) ?? (minutes > startHour * 60 ? format(time, labelFormat, { locale: settings.locale }) : null)
        return (
          <div key={minutes} className="relative" style={{ height: `calc(var(--ec-hour-height) * ${interval / 60})` }}>
            {label != null && (
              <span
                className={cn('text-muted-foreground absolute end-2.5 -top-2', viewConfig.classNames?.timeGutterLabel)}
              >
                {label}
              </span>
            )}
...
function EventCalendarDayColumn({
  ...
  const windowFor = (start: Date, end: Date): [number, number] | null => {
    ...
    const from = Math.max(differenceInMinutes(start > dayStart ? start : dayStart, dayStart), boundsStartMin)
  ...
  const dragGhost = useEventCalendarSelector<
    unknown,
    | (Pick<EventCalendarDragState, 'valid' | 'kind' | 'occurrence' | 'proposedStart' | 'proposedEnd'> & {
  ...
  const slotFromPointer = (e: React.MouseEvent): { date: Date; end: Date } => {
    ...
    const minutes = snapMinutes(boundsStartMin + (e.clientY - rect.top) / pxPerMinute, settings.snapDuration)
  ...
  return (
    <div
      ...
      role="group"
      aria-label={format(toZoned(day, timeZone), settings.i18n.formats.dayAria, { locale: settings.locale })}
      className={cn(
