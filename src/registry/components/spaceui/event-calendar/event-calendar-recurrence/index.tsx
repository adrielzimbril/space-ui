...
function formatRRuleString(rule: EventCalendarRecurrenceRule): string {
  ...
  if (rule.byWeekday?.length) {
    parts.push(`BYDAY=${rule.byWeekday.map((d) => (typeof d === 'string' ? d : `${d.ordinal}${d.day}`)).join(',')}`)
  }
...
function resolveRule(recurrence: EventCalendarRecurrenceRule | string, timeZone?: string): EventCalendarRecurrenceRule {
...
function expandRecurrence<TData>(
  ...
  if (!event.recurrence) {
    ...
    if (event.start < range.end && (event.end > range.start || (isPoint && event.start >= range.start))) {
      return [
        {
  ...
  const wallDaySpan = daySpan > 0 && addDays(zonedStart, daySpan).getTime() === event.end.getTime() ? daySpan : null
  ...
  const monthlyByDay =
    (rule.freq === 'monthly' || rule.freq === 'yearly') && rule.byWeekday?.length ? rule.byWeekday : null
  ...
  const yearlyMonths: number[] =
    validByMonth.length > 0 ? [...new Set(validByMonth)].sort((a, b) => a - b) : [zonedStart.getMonth() + 1]
  ...
  const hasMonthDayParts =
    ...
    (rule.freq === 'yearly' && Boolean(rule.byMonth?.length || rule.byMonthDay?.length || monthlyByDay))
  ...
  const monthDays = (year: number, month: number): number[] => {
    ...
    if (rule.byMonthDay?.length) {
      ...
      days = rule.byMonthDay.map((n) => (n < 0 ? total + 1 + n : n)).filter((n) => n >= 1 && n <= total)
    }
    if (monthlyByDay) {
...
export { EventCalendarRecurrenceError, expandRecurrence, formatRRuleString, MAX_OCCURRENCES, parseRRuleString }
