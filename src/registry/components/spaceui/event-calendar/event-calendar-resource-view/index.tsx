...
import {
  ...
  minuteBlockStyle,
} from '@/registry/components/spaceui/event-calendar/event-calendar-time-grid'
import type {
...
function EventCalendarResourceView({
  ...
  const allDaySegments = useEventCalendarSelector<unknown, EventCalendarSegment[]>(
    () =>
      viewConfig.renderAllDaySection
        ? (instance.internals.getIndex().byDay.get(getDayKey(day, settings.timeZone))?.allDay ?? EMPTY_ALL_DAY_SEGMENTS)
  ...
  const track = (
    <div className="relative flex">
      ...
      {effective.nowIndicator && <EventCalendarNowIndicator days={[day]} startHour={startHour} endHour={endHour} />}
    </div>
  )
...
function EventCalendarResourceAllDayCell({ resource, day }: { resource: EventCalendarResource; day: Date }) {
  const settings = useEventCalendarSettings()
  const viewConfig = useEventCalendarViewConfig()
  const { effective } = useEventCalendarViewSettings()
...
function EventCalendarResourceColumn({
  ...
  >(
    (state) => {
      ...
      const from = Math.max((drag.proposedStart.getTime() - dayStart.getTime()) / 60000, boundsStartMin)
