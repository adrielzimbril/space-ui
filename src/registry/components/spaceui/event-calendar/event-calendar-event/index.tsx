...
interface EventCalendarEventProps<TData = unknown> extends Omit<useRender.ComponentProps<'button'>, 'children'> {
  segment: EventCalendarSegment<TData>
...
function EventCalendarEvent<TData = unknown>({
  ...
  // into neighbors
  const stackedBlock = timedBlock && (segment.endMin ?? 0) - (segment.startMin ?? 0) >= viewConfig.compactEventMinutes
  ...
  const defaultContent = (
    <>
      ...
      {occurrence.isRecurring && <IconRepeat className="size-2.5 shrink-0 opacity-70" aria-hidden="true" />}
      <span className={cn('font-medium', stackedBlock ? EVENT_CALENDAR_FADE_TRUNCATE : 'truncate')}>{event.title}</span>
  ...
  const agendaDefaultContent = (
    <>
      <span className="text-muted-foreground w-40 shrink-0 truncate tabular-nums">{agendaTimeText}</span>
      <span
        aria-hidden
      ...
      {occurrence.isRecurring && <IconRepeat className="text-muted-foreground size-2.5 shrink-0" aria-hidden="true" />}
    </>
  )
  ...
  const customContent = useMemo(() => {
    ...
    return view === 'agenda' ? viewConfig.renderAgendaEvent?.(renderProps) : viewConfig.renderEvent?.(renderProps)
  }, [occurrence, segment, view, isDragging, isSelected, viewConfig.renderAgendaEvent, viewConfig.renderEvent])
  const content = children ?? customContent ?? (view === 'agenda' ? agendaDefaultContent : defaultContent)
  ...
  const defaultProps = {
    ...
    'aria-label':
      ...
      `${event.title}, ${timeLabel}${
        segment.continuesBefore || segment.continuesAfter ? `, ${settings.i18n.labels.continues}` : ''
      }`,
      ...
      {tooltipOn ? (
        <TooltipProvider delay={tooltipOpts?.delay ?? 600}>
          <Tooltip>
            ...
            <TooltipContent side={tooltipOpts?.side ?? 'top'} className={viewConfig.classNames?.eventTooltip}>
              {tooltipContent}
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
