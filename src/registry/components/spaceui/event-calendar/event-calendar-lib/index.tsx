...
import { expandRecurrence } from '@/registry/components/spaceui/event-calendar/event-calendar-recurrence'
import type {
  CalendarEvent,
  CalendarView,
...
function packWeekRowLanes<TData>(
  ...
  const bars: Bar[] = segments.map((seg) => {
    const dayIndex = Math.round(
      (zonedStartOfDay(seg.day, timeZone).getTime() - zonedStartOfDay(rowStart, timeZone).getTime()) /
        (24 * 60 * 60 * 1000),
    )
    return {
      seg,
  ...
  const rowBars = Array.from(merged.values()).sort(
    (a, b) =>
      a.colStart - b.colStart || b.colSpan - a.colSpan || a.seg.occurrence.key.localeCompare(b.seg.occurrence.key),
  )
...
function buildEventIndex<TData>(
  ...
  for (const event of events) {
    ...
    occurrences.push(...(replaced ? expanded.filter((occ) => !replaced.has(occ.start.getTime())) : expanded))
  }
  ...
  for (const occurrence of occurrences) {
    ...
    for (const seg of segments) {
      ...
      if (bar) {
        ...
        const rowIndex = Math.floor(differenceInCalendarDays(toZoned(seg.day, timeZone), firstRowStart) / 7)
