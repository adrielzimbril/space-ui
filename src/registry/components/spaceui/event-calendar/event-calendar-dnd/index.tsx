...
import { snapMinutes, toZoned, zonedStartOfDay } from '@/registry/components/spaceui/event-calendar/event-calendar-lib'
import type {
...
function beginGesture<TData>(config: BeginGestureConfig<TData>) {
  ...
  const gestureCursor = kind.startsWith('resize') ? (isBar ? 'ew-resize' : 'ns-resize') : 'grabbing'
    ...
    if (kind === 'move' && occurrence) {
      ...
      if (surface.columns.length > 0 && !isBar) {
        ...
        if (col) {
          ...
          grabOffsetMin = pointerMinutes(surface, col, startY) - (segStartMs - colDayStart.getTime()) / 60000
        }
      }
    }
  ...
  const CARRY_INVALID_CLASS =
    `${CARRY_CLASS} ring-destructive/60 ring-1` + (ui?.dragCarryInvalid ? ' ' + ui.dragCarryInvalid : '')
