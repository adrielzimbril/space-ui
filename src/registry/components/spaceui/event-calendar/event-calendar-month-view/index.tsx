...
} from '@/registry/components/spaceui/event-calendar/event-calendar'
import {
  useEventCalendarGestures,
  wasRecentChipPress,
  wasRecentDrag,
} from '@/registry/components/spaceui/event-calendar/event-calendar-dnd'
import {
  EVENT_CALENDAR_GHOST,
  EventCalendarEvent,
} from '@/registry/components/spaceui/event-calendar/event-calendar-event'
import {
  getDayKey,
  getRangeKey,
  resolveOffDay,
...
function restoreChipFocus(root: HTMLElement | null, segments: EventCalendarSegment[]) {
  ...
  const index = segments.findIndex(
    (segment) =>
      segment.occurrence.eventId === pending.eventId && segment.occurrence.recurrenceIndex === pending.recurrenceIndex,
  )
  if (index < 0) return
...
function EventCalendarMonthView({ className, render, maxEventsPerCell, ...props }: EventCalendarMonthViewProps) {
  const instance = useEventCalendar()
  const settings = useEventCalendarSettings()
  const viewConfig = useEventCalendarViewConfig()
  const visibleRange = useEventCalendarSelector<unknown, EventCalendarDateRange>((state) => state.visibleRange, {
          ...
          {headerDays.map((day) => (
            ...
            >
              {viewConfig.renderDayHeader?.({
                day,
                view: 'month',
                isToday: getDayKey(day, settings.timeZone) === getDayKey(new Date(), settings.timeZone),
              }) ?? (
                <>
                  ...
                  <span className="hidden @max-[36rem]:inline">
                    {format(toZoned(day, settings.timeZone), settings.i18n.formats.monthDayHeaderNarrow, {
                      locale: settings.locale,
                    })}
                  </span>
                </>
...
function EventCalendarMonthWeek({
  ...
  const offsets = week.map((d) => Math.round((zonedStartOfDay(d, settings.timeZone).getTime() - rowStartMs) / dayMs))
  ...
  const hiddenBarKeysByCol = week.map(
    (_, col) =>
      new Set(bars.filter((b) => (b.lane ?? 0) >= cap && covers(b, offsets[col])).map((b) => b.occurrence.key)),
  )
  ...
  const ghostLane = dragGhost ? (bars.find((b) => b.occurrence.key === dragGhost.occurrence.key)?.lane ?? 0) : 0
  ...
  return (
    <div
      ...
      className={cn('relative grid min-h-0 border-b last:border-b-0', viewConfig.classNames?.monthRow)}
      style={{ gridTemplateColumns }}
    >
      {showWeekNumber && (
        <div
          ...
          className={cn('text-muted-foreground border-e px-2 pt-1 tabular-nums', viewConfig.classNames?.weekNumber)}
        >
          ...
          {visibleBars.map((bar) => {
            ...
            return (
              ...
              >
                ...
                <EventCalendarEvent segment={bar} className="h-[calc(var(--ec-month-bar-h,1.75rem)-0.125rem)]" />
              </div>
            )
          })}
              ...
              <div
                ...
                className={cn(
                  ...
                  dragGhost.kind === 'move'
                    ...
                    : cn(EVENT_CALENDAR_GHOST.resize, !dragGhost.valid && EVENT_CALENDAR_GHOST.invalidResize),
              ...
              >
                {dragGhost.kind !== 'move' && (
                  <EventCalendarEvent
                    ...
                    className={cn('h-full inset-ring-0', !dragGhost.valid && EVENT_CALENDAR_GHOST.invalidContent)}
                  />
                )}
              </div>
            </div>
...
function EventCalendarMonthCell({
  ...
  const isOff = resolveOffDay(
    day,
    settings.timeZone,
    effective.offDays ? (typeof viewConfig.offDays === 'object' ? viewConfig.offDays : true) : false,
    settings.weekendDays,
  )
  const offClassName = (typeof viewConfig.offDays === 'object' && viewConfig.offDays.className) || 'bg-muted/25'
  ...
  const inDraft = useEventCalendarSelector<unknown, { isStart: boolean; isEnd: boolean } | null>(
    ...
    {
      isEqual: (a, b) => a === b || (a !== null && b !== null && a.isStart === b.isStart && a.isEnd === b.isEnd),
    },
  )
  ...
  // the spot).
  const inlineDrop = useEventCalendarSelector<unknown, { min: number; valid: boolean; color?: string } | null>(
    (state) => {
      const drag = state.drag
      ...
      if (zonedStartOfDay(drag.occurrence.start, settings.timeZone).getTime() === dayStart.getTime()) return null
      return {
    ...
    {
      isEqual: (a, b) =>
        a === b || (a !== null && b !== null && a.min === b.min && a.valid === b.valid && a.color === b.color),
    },
  )
  ...
  if (!inlineDrop) {
    ...
    visibleTimed = placeholderAtMore ? segments.timed.slice(0, vis) : segments.timed.slice(0, Math.max(0, vis - 1))
      ...
      >
        ...
        {overflowCount > 0 && (
          <EventCalendarMoreIndicator
            ...
            dropInto={
              placeholderAtMore && inlineDrop ? { color: inlineDrop.color, valid: inlineDrop.valid } : undefined
            }
          />
        )}
      </div>
      {/* Day number + add affordance, bottom-right (Notion-style) */}
      <div className={cn('flex items-center justify-end gap-1 px-2 pb-1.5', viewConfig.classNames?.monthCellFooter)}>
  ...
  return (
    <div
      ...
      aria-label={format(toZoned(day, settings.timeZone), settings.i18n.formats.monthCellAriaLabel, {
        locale: settings.locale,
      })}
      className={cn(
        ...
        isOff && offClassName,
        isToday && cn('bg-primary/3 border-b-primary/40 relative border-b-2', viewConfig.todayClassName),
 ...
 */
function EventCalendarMoreIndicator({ day, count, segments, dropInto }: EventCalendarMoreIndicatorProps) {
  const settings = useEventCalendarSettings()
  const viewConfig = useEventCalendarViewConfig()
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        {viewConfig.renderMoreIndicator?.({ day, count, segments }) ?? settings.i18n.labels.more(count)}
      </PopoverTrigger>
      <PopoverContent
        ...
        aria-label={
          viewConfig.renderMoreContent
            ? format(toZoned(day, settings.timeZone), settings.i18n.formats.moreDayHeader, { locale: settings.locale })
            : undefined
        }
...
function EventCalendarMoreDefaultContent({
  ...
  return (
    <>
      <div
        id={headerId}
        className={cn('text-muted-foreground px-1 py-1 text-xs font-medium', viewConfig.classNames?.morePopoverHeader)}
      >
        {format(toZoned(day, settings.timeZone), settings.i18n.formats.moreDayHeader, { locale: settings.locale })}
      </div>
      ...
      {viewConfig.scrollbars === 'native' ? (
        <div data-ec-native-scroll="" className="-me-2 max-h-(--ec-more-max-height,16rem) min-h-0 overflow-y-auto">
