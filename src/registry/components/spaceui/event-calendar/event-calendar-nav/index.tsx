...
import { toZoned } from '@/registry/components/spaceui/event-calendar/event-calendar-lib'
import type { CalendarView } from '@/registry/components/spaceui/event-calendar/event-calendar-types'
...
import { Menu, MenuPopup, MenuGroup, MenuItem, MenuGroupLabel, MenuTrigger } from '@/registry/primitives/menu'
...
function NavTooltip({ content, children }: { content: ReactNode | null; children: React.ReactElement }) {
...
function EventCalendarNavToday({ className, render, children, tooltip, ...props }: NavButtonProps) {
  ...
  const defaultTooltip = format(toZoned(new Date(), settings.timeZone), settings.i18n.formats.dayTitle, {
...
function EventCalendarTitle({ className, render, format: formatTitle, ...props }: EventCalendarTitleProps) {
  const { title } = useEventCalendarNavigation()
  const viewConfig = useEventCalendarViewConfig()
  const defaultProps = {
    'data-slot': 'event-calendar-title',
    'aria-live': 'polite' as const,
    className: cn('min-w-0 truncate text-sm font-semibold', viewConfig.classNames?.title, className),
...
interface EventCalendarViewSwitcherProps extends Omit<useRender.ComponentProps<'button'>, 'children'> {
  children?: ReactNode
...
function EventCalendarViewSwitcher({ className, render, children, tooltip, ...props }: EventCalendarViewSwitcherProps) {
  ...
  return (
    ...
    >
      ...
      <MenuPopup align="start" className={cn('min-w-44', viewConfig.classNames?.viewSwitcherContent)}>
        {/* Base UI contract: GroupLabel must live inside Menu.Group */}
        <MenuGroup>
          <MenuGroupLabel className={cn('text-muted-foreground font-normal', viewConfig.classNames?.viewSwitcherLabel)}>
            ...
            v === 'days' ? (
              viewConfig.dayCountPresets.map((count) => (
                ...
                >
                  ...
                  {viewConfig.enableShortcuts && <EventCalendarViewShortcut>{count}</EventCalendarViewShortcut>}
                </MenuItem>
              ))
            ) : (
              <MenuItem key={v} data-active={view === v || undefined} onClick={() => selectView(v)}>
                {viewLabel(v)}
...
interface EventCalendarDatePickerProps extends Omit<useRender.ComponentProps<'button'>, 'children'> {
  children?: ReactNode
...
function EventCalendarDatePicker({
  ...
  const resolved = configured === 'auto' ? (RANGE_VIEWS.includes(view) ? 'range' : 'single') : configured
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" className={cn('w-auto p-0!', viewConfig.classNames?.datePickerContent)}>
        {resolved === 'range' ? (
          <Calendar
            mode="range"
            selected={{
 ...
 */
function EventCalendarNav({ className, render, children, showViewSwitcher = true, ...props }: EventCalendarNavProps) {
  const viewConfig = useEventCalendarViewConfig()
  const tooltips = useNavTooltipConfig()
  const defaultProps = {
    ...
    children: children ?? (
      ...
      <TooltipProvider delay={tooltips.delay} closeDelay={tooltips.closeDelay} timeout={tooltips.timeout}>
