...
import { EventCalendarEvent } from '@/registry/components/spaceui/event-calendar/event-calendar-event'
import {
  getDayKey,
  getRangeKey,
  toZoned,
...
function EventCalendarAgendaView({ className, render, ...props }: EventCalendarAgendaViewProps) {
  ...
  const isToday = (day: Date) => getDayKey(day, settings.timeZone) === getDayKey(new Date(), settings.timeZone)
          ...
          {groups.map(({ day, bucket }) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span className={cn('text-foreground font-semibold', isToday(day) && 'text-primary')}>{weekday}</span>
