...
import { IconCheck, IconEye, IconInfoCircle, IconX } from '@tabler/icons-react'
        ...
        <InputGroup>
          ...
          <InputGroupAddon align="inline-end">
            <Button
              ...
              variant="ghost"
            >
              {isVisible ? <IconInfoCircle aria-hidden="true" /> : <IconEye aria-hidden="true" />}
            </Button>
          </InputGroupAddon>
        </InputGroup>
      </div>
      ...
      <ul aria-label="Password requirements" className="flex flex-col gap-1.5">
        {strength.map((req) => (
          <li className="flex items-center gap-2" key={req.text}>
            {req.met ? (
              <IconCheck aria-hidden="true" className="size-4 text-emerald-500" />
            ) : (
              <IconX aria-hidden="true" className="size-4 text-muted-foreground/80" />
            )}
