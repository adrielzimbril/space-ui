...
import { IconCopy, IconPlus, IconSearch, IconX } from '@tabler/icons-react'
  ...
  return (
    ...
    >
      <ComboboxTrigger
        ...
        render={<Button className="w-24 font-normal tabular-nums" id={id} size="sm" variant="outline" />}
      >
        <ComboboxValue />
      </ComboboxTrigger>
    ...
    >
      ...
      <PopoverPopup align="end" className="w-44">
        <div className="flex flex-col gap-3">
          <div className="font-medium text-foreground text-sm">Copy times to</div>
          <CheckboxGroup aria-label={`Copy ${day} times to`} onValueChange={setSelectedDays} value={selectedDays}>
            {days
...
export default function Particle() {
  ...
  const addRange = (day: Day) => {
    ...
    setDayRanges(day, [...ranges, createRange(timeOptions[startIndex] ?? '', timeOptions[endIndex] ?? '')])
  }
  ...
  const copyTo = (source: Day, targets: Day[]) => {
    setAvailability((prev) => {
      ...
      for (const target of targets) {
        next[target] = prev[source].map((range) => createRange(range.start, range.end))
      }
      return next
    })
  }
            ...
            >
              ...
              <div className="flex w-full min-w-0 items-start gap-4 md:flex-1">
                <div className="flex min-w-0 flex-col gap-2">
                  {ranges.length === 0 ? (
                    <p className="flex h-8 items-center text-muted-foreground sm:h-7 sm:text-sm">Unavailable</p>
                  ) : (
                    ranges.map((range) => {
                      ...
                      return (
                        <div className="flex items-center gap-2" key={range.id}>
                          <Group aria-label={`${day} time range`}>
                            <GroupText render={<Label htmlFor={startId} />}>From</GroupText>
                            <GroupSeparator />
                            <TimeCombobox
                              ariaLabel={`${day} start time`}
                              id={startId}
                              items={timeOptions}
                              onChange={(start) => updateStart(day, range.id, start)}
                              value={range.start}
                            />
                            <GroupSeparator />
                            <GroupText render={<Label htmlFor={endId} />}>To</GroupText>
                            <GroupSeparator />
                            <TimeCombobox
