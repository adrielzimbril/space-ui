...
function FeatureCard({
  ...
  asHandle,
  ...props
}: { feature: Feature; asHandle?: boolean } & Omit<ComponentProps<typeof KanbanItem>, 'value' | 'children'>) {
  const content = (
    <Frame variant="ghost" spacing="sm" className="p-0">
      <FramePanel className="p-3">
        <div className="flex flex-col gap-2.5">
          <span className="text-sm font-medium">{feature.title}</span>
          <p className="text-muted-foreground line-clamp-2 text-xs">{feature.description}</p>
          <Progress value={feature.progress} className="h-1.5" />
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground text-[10px] tabular-nums">{feature.progress}% complete</span>
            <div className="flex items-center gap-1">
              <IconArrowUp className="text-muted-foreground size-3" />
              <span className="text-muted-foreground text-xs tabular-nums">{feature.votes}</span>
            </div>
          </div>
        </div>
      </FramePanel>
          ...
          return (
            <KanbanColumn key={colId} value={colId}>
              <Frame spacing="sm" className="h-full">
                ...
                <KanbanColumnContent value={colId} className="flex flex-col gap-2 p-0.5">
