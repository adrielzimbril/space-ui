...
} from '@/registry/components/spaceui/date-selector'
...
import { Menu, MenuPopup, MenuItem, MenuTrigger } from '@/registry/primitives/menu'
// Helper function to create i18n config from translations
function createI18nConfig(translations: Partial<DateSelectorI18nConfig>): DateSelectorI18nConfig {
...
export default function Pattern() {
  ...
  const [currentLanguage, setCurrentLanguage] = useState<keyof typeof languageMetadata>('fr')

  // Get current language metadata and i18n config
  const currentMeta = useMemo(() => languageMetadata[currentLanguage] || languageMetadata.en, [currentLanguage])

  const currentI18n = useMemo(() => i18nConfigs[currentLanguage] || i18nConfigs.en, [currentLanguage])
        ...
        <Menu>
          ...
          <MenuPopup align="start">
            {languageOptions.map((lang) => (
              <MenuItem
                key={lang.value}
                onClick={() => setCurrentLanguage(lang.value as keyof typeof languageMetadata)}
