...
import { EventCalendarContent } from '@/registry/components/spaceui/event-calendar/event-calendar-content'
import type { EventCalendarI18nOverrides } from '@/registry/components/spaceui/event-calendar/event-calendar-i18n'
import { EventCalendarNav, EventCalendarToolbar } from '@/registry/components/spaceui/event-calendar/event-calendar-nav'
import type {
  CalendarEvent,
  CalendarView,
...
import { addDays, addMinutes, setHours, startOfDay, startOfWeek, type Locale } from 'date-fns'
      ...
      <>
        <span className="flex shrink-0 -space-x-1">
          <Avatar className="ring-background size-4 ring-1">
            <AvatarFallback className="bg-violet-500 text-[8px] font-semibold text-white">MJ</AvatarFallback>
          </Avatar>
          <Avatar className="ring-background size-4 ring-1">
            <AvatarFallback className="bg-sky-500 text-[8px] font-semibold text-white">AL</AvatarFallback>
          </Avatar>
        </span>
  ...
  if (event.id === 'client-call') {
    return (
      <span className="flex w-full min-w-0 items-center gap-1.5">
        ...
        <span className="truncate font-medium">{event.title}</span>
        <span className="ms-auto shrink-0 rounded bg-(--ec-event-color)/25 px-1 text-[10px] font-semibold">30m</span>
      </span>
    )
  }
...
export default function Demo() {
  ...
  const activeTimeZone = TIME_ZONES.find((entry) => entry.id === settings.timeZoneId) ?? TIME_ZONES[0]

  const patch = (partial: Partial<DemoSettings>) => setSettings((current) => ({ ...current, ...partial }))
                    ...
                    <Tabs defaultValue="view">
                      ...
                      <TabsContent value="view" className="flex flex-col gap-3">
                        ...
                        <SettingsSelect
                          ...
                          onValueChange={(weekStartsOn) => patch({ weekStartsOn: weekStartsOn as 0 | 1 })}
                        />
                      </TabsContent>
                      <TabsContent value="time" className="flex flex-col gap-3">
                        <SettingsSelect
                          ...
                          onValueChange={(dayStartHour) => patch({ dayStartHour })}
                        />
                        <SettingsSelect
                          id="ec-set-day-end"
                          label="Day ends"
                        ...
                        <SettingsSelect
                          ...
                          onValueChange={(snapDuration) => patch({ snapDuration })}
                        />
                      </TabsContent>
                      ...
                      <TabsContent value="region" className="flex flex-col gap-3">
                        ...
                        <p className="text-muted-foreground text-xs leading-relaxed">
                          Language switches the date-fns locale and every UI label. Time zone shifts all event times.
                          Arabic also flips the calendar to right-to-left.
                        </p>
                      </TabsContent>
                    </Tabs>
                    <Button
