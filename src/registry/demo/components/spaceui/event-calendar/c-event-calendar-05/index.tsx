...
function buildAppointments(anchor: Date): CalendarEvent<ApptData>[] {
  ...
  return [
    appt(
      ...
      'Dana Whitfield',
      'DW',
      'https://avatars.spaceui.one/v1?name=orion&variant=lumina',
      'consultation',
      9,
      0,
      45,
    ),
    appt('a2', 'Marco Reyes', 'MR', 'https://avatars.spaceui.one/v1?name=lyra&variant=shaula', 'followup', 11, 0, 45),
    appt(
      'a3',
      'Priya Nair',
      'PN',
      'https://avatars.spaceui.one/v1?name=vega&variant=singularity',
      'assessment',
      14,
      0,
      60,
    ),
    appt('a4', 'Leon Fischer', 'LF', 'https://avatars.spaceui.one/v1?name=sirius&variant=triton', 'therapy', 16, 0, 45),
  ]
}
...
function renderChip({ occurrence, segment }: EventCalendarRenderEventProps<ApptData>) {
  ...
  return (
    <span className="flex h-full w-full min-w-0 flex-col justify-start gap-0.5">
      ...
      {service && minutes >= 45 && (
        <span className="truncate ps-[1.625rem] text-[11px] font-medium text-(--ec-event-color)">{service.label}</span>
      )}
    </span>
  )
}
          ...
          >
            <div className="flex flex-wrap items-center gap-2 pe-2">
              ...
              <EventCalendarToolbar>
                <Button variant="outline" size="sm" onClick={() => apiRef.current?.setView('agenda')}>
