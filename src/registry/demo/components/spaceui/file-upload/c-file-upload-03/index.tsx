...
import { IconPlus, IconFile, IconX, IconAlertCircle } from '@tabler/icons-react'
    ...
    <div className={cn('w-full max-w-lg', className)}>
      ...
      >
        ...
        <div className="flex flex-1 items-center gap-2">
          {files.length === 0 ? (
            <p className="text-muted-foreground text-sm">Drop files here or click to browse (max {maxFiles} files)</p>
          ) : (
            files.map((fileItem) => (
              <div key={fileItem.id} className="group/item relative shrink-0">
                {isImage(fileItem.file) && fileItem.preview ? (
                  ...
                  <div
                    ...
                    title={`${fileItem.file.name} (${formatBytes(fileItem.file.size)})`}
                  >
                    <IconFile className="text-muted-foreground h-5 w-5" />
                  </div>
                )}
