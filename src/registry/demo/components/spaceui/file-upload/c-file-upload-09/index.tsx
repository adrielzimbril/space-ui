...
import { useFileUpload, type FileMetadata, type FileWithPreview } from '@/registry/hooks/form/use-file-upload'
...
export default function Demo({
  ...
  const getFileIcon = (file: File | FileMetadata) => {
    ...
    if (type.startsWith('image/')) return <IconPhoto className="text-muted-foreground size-4" />
    if (type.startsWith('video/')) return <IconVideo />
    if (type.startsWith('audio/')) return <IconHeadphones className="size-6" />
    if (type.includes('pdf')) return <IconFileText className="size-6" />
    if (type.includes('word') || type.includes('doc')) return <IconFileText className="size-6" />
    if (type.includes('excel') || type.includes('sheet')) return <IconFileSpreadsheet className="size-6" />
    if (type.includes('zip') || type.includes('rar')) return <IconFileZip className="size-6" />
    return <IconFileText />
  }
                      ...
                      <div className="relative flex items-center justify-between gap-2">
                        ...
                        {fileItem.status === 'error' && fileItem.error && (
                          <Tooltip>
                            ...
                            >
                              <IconRefreshDot className="size-3 opacity-100" />
                            </TooltipTrigger>
