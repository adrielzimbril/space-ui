...
import { IconCalendarEvent, IconClock, IconAdjustmentsAlt, IconFilter2, IconFilterX } from '@tabler/icons-react'
...
function CustomModalDateSelector({ values, onChange, autoFocus }: CustomRendererProps) {
...
function CustomDateRangeInput({ values, onChange, autoFocus }: CustomRendererProps) {
...
function CustomDateRangeWithPresetsInput({ values, onChange, autoFocus }: CustomRendererProps) {
...
function CustomDateTimeInput({ values, onChange, autoFocus }: CustomRendererProps) {
...
function CustomSliderRangeInput({ values, onChange, autoFocus }: CustomRendererProps) {
...
export default function Demo() {
  ...
  const fields: FilterFieldConfig[] = [
    {
      ...
      customRenderer: ({ values, onChange }) => (
        <CustomModalDateSelector values={values} onChange={onChange} autoFocus={values === lastAddedValues} />
      ),
    },
    {
      key: 'customDateRange',
      ...
      customRenderer: ({ values, onChange }) => (
        <CustomDateRangeInput values={values} onChange={onChange} autoFocus={values === lastAddedValues} />
      ),
    },
    {
      ...
      customRenderer: ({ values, onChange }) => (
        <CustomDateRangeWithPresetsInput values={values} onChange={onChange} autoFocus={values === lastAddedValues} />
      ),
    },
    {
      key: 'customDateTime',
      ...
      customRenderer: ({ values, onChange }) => (
        <CustomDateTimeInput values={values} onChange={onChange} autoFocus={values === lastAddedValues} />
      ),
    },
    {
      key: 'customSliderRange',
      label: 'Slider Range',
      icon: <IconAdjustmentsAlt className="size-3.5" />,
      type: 'custom',
      className: 'w-36',
      operators: [
        { value: 'between', label: 'between' },
      ...
      customRenderer: ({ values, onChange }) => (
        <CustomSliderRangeInput values={values} onChange={onChange} autoFocus={values === lastAddedValues} />
      ),
    },
  ]
  ...
  const handleFiltersChange = useCallback(
    (newFilters: Filter[]) => {
      ...
      const added = newFilters.find((nf) => !filters.some((f) => f.id === nf.id))
      if (added) {
