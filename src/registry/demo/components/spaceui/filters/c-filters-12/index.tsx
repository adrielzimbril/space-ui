...
export default function Demo() {
    ...
    () => [
      {
        key: 'assignee',
        label: 'Assignee',
        icon: <IconUserSearch className="size-3.5" />,
        type: 'multiselect',
        ...
        loadOptions: async (query: string) => {
          ...
          const matches = q ? DIRECTORY.filter((option) => option.label.toLowerCase().includes(q)) : DIRECTORY
  ...
  const [filters, setFilters] = useState<Filter[]>([createFilter('assignee', 'is_any_of', ['user-1'])])
