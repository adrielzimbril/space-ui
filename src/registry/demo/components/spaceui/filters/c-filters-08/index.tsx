...
import { createParser, parseAsBoolean, parseAsJson, useQueryState, useQueryStates } from 'nuqs'
...
const AvailabilityStatus = ({ availability }: { availability: string }) => {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-muted-foreground text-sm">{getStatusLabel(availability)}</span>
    </div>
  )
}
...
export default function Demo() {
  ...
  const [sorting, setSorting] = useQueryState(
    'sort',
    parseAsJson<SortingState>((v) => v as SortingState).withDefault([{ id: 'name', desc: false }]),
  )
  ...
  const filters = useMemo(() => {
    return Object.entries(filterStates)
      .filter(([key, state]) => key !== 'filters' && state !== null && typeof state !== 'boolean')
      .map(([key, state]) => {
        ...
        {filters.length > 0 && (
          <Button
            ...
            onClick={() => {
              ...
              Object.keys(filterStates).forEach((key) => (clearedStates[key] = null))
