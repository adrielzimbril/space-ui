...
const roadmapItems = [
  ...
  },
  {
    ...
    content: (
      <>
        <span className="text-muted-foreground">Completed</span> Usability Testing
      </>
    ),
    color: 'bg-violet-500',
  },
  ...
  },
  {
    ...
    content: (
      <>
        <span className="text-muted-foreground">Completed</span> Requirements Gathering
      </>
    ),
    color: 'bg-blue-500',
  },
        ...
        {roadmapItems.map((item, index) => (
          ...
          >
            <TimelineHeader className="flex items-center gap-2.5">
              ...
              <TimelineIndicator className={cn('size-2 border-none', item.color)} />
              <TimelineDate className="text-muted-foreground/60 mb-0 text-[10px] font-semibold uppercase">
                {item.date}
              </TimelineDate>
            </TimelineHeader>
            <TimelineContent className="text-foreground text-sm font-medium">{item.content}</TimelineContent>
          </TimelineItem>
        ))}
      </Timeline>
    </div>
